...
unless (($opts{'l'}) || ($opts{'n'}) || ($opts{'N'})) {
	die "-n: search by expansion, -N: seach by expansion list, -q: print only summary, -u: print id, -s: skip summary, -p: include prices, -l: print out aliases.\n";
}
...
foreach $exp (@exps) {
	...
	if (($opts{'C'}) || ($opts{'U'}) || ($opts{'R'}) || ($opts{'M'}) || ($opts{'L'})) {
		$do_grep = 1;
	}
